...
func (v *VNA) SetupAdapter() error {
	...
	if _,err := services.RunCommand(ctx, 
		"netsh", "interface", "ip", "set", "address",nameArg, "static", v.IP, v.Mask,); err != nil {
		return fmt.Errorf("setup IP failed: %w", err)
	}

	// ============ GET ADAPTER INDEX ============
	out, err := services.RunCommand(ctx, "netsh", "interface", "ipv4", "show", "interfaces")
	...
	if _,err := services.RunCommand(ctx,
		...
		"0.0.0.0/0", strconv.Itoa(v.AdapterIndex),"metric=1",); err != nil {
	...
	if _,err := services.RunCommand(ctx, 
    	...
    	fmt.Sprintf("%q", v.IfName), "mtu=1400", "store=persistent",); err != nil{
...
func (v *VNA) RemoveRoute() error {
	...
	// use background context to ensure route cleanup even on shutdown
    ctxDel, cancel := context.WithTimeout(context.Background(), 5*time.Second)
    defer cancel()


	_,err := services.RunCommand(ctxDel,"netsh", "interface", "ipv4", "delete", "route","0.0.0.0/0", strconv.Itoa(v.AdapterIndex),)
    return err
}
